...
Y8950TimAdpcm::Y8950TimAdpcm(Y8950Tim& y8950_, const DeviceConfig& config,
...
void Y8950TimAdpcm::clearRam()
{
	ram.clear(0xFF);
}

void Y8950TimAdpcm::reset(EmuTime::param time)
{
	removeSyncPoint();
	...
	y8950.setStatus(Y8950Tim::STATUS_BUF_RDY);
}

bool Y8950TimAdpcm::isPlaying() const
{
	return (reg7 & 0xC0) == 0x80;
}
bool Y8950TimAdpcm::isMuted() const
{
	return !isPlaying() || (reg7 & R07_SP_OFF);
}

void Y8950TimAdpcm::restart(PlayData& pd) const
{
	pd.memPtr = startAddr;
...
void Y8950TimAdpcm::sync(EmuTime::param time)
{
...
void Y8950TimAdpcm::schedule()
{
	...
	if ((stopAddr > startAddr) && (delta != 0)) {
		...
		if (reg7 & R07_MEMORY_DATA) {
			...
			Clock<Y8950Tim::CLOCK_FREQ, Y8950Tim::CLOCK_FREQ_DIV> stop(clock);
...
void Y8950TimAdpcm::executeUntil(EmuTime::param time)
{
	assert(isPlaying());
	sync(time); // should set STATUS_EOS
	assert(y8950.peekRawStatus() & Y8950Tim::STATUS_EOS);
	if (isPlaying() && (reg7 & R07_REPEAT)) {
		schedule();
	}
}

void Y8950TimAdpcm::writeReg(byte rg, byte data, EmuTime::param time)
{
	...
	case 0x07: // START/REC/MEM DATA/REPEAT/SP-OFF/-/-/RESET
		reg7 = data;
		if (reg7 & R07_START) {
			y8950.setStatus(Y8950Tim::STATUS_PCM_BSY);
		} else {
			y8950.resetStatus(Y8950Tim::STATUS_PCM_BSY);
		}
		if (reg7 & R07_RESET) {
			reg7 = 0;
		}
...
void Y8950TimAdpcm::writeData(byte data)
{
	reg15 = data;
	if ((reg7 & R07_MODE) == 0x60) {
		...
		if (emu.memPtr <= stopAddr) {
			...
			y8950.resetStatus(Y8950Tim::STATUS_BUF_RDY);

			// setup a timer that will callback us in 10
			// master clock cycles for Y8950Tim. In the
			...
			if (emu.memPtr > stopAddr) {
				...
				y8950.setStatus(Y8950Tim::STATUS_EOS);
...
byte Y8950TimAdpcm::readReg(byte rg, EmuTime::param time)
{
	sync(time); // TODO only when needed
	byte result = (rg == 0x0F)
	            ? readData()   // ADPCM-DATA
...
byte Y8950TimAdpcm::peekReg(byte rg, EmuTime::param time) const
{
	const_cast<Y8950TimAdpcm*>(this)->sync(time); // TODO only when needed
	return peekReg(rg);
}

byte Y8950TimAdpcm::peekReg(byte rg) const
{
	switch (rg) {
	case 0x0F: // ADPCM-DATA
		return peekData();
...
void Y8950TimAdpcm::resetStatus()
{
...
byte Y8950TimAdpcm::peekData() const
{
	if ((reg7 & R07_MODE) == R07_MEMORY_DATA) {
		// external memory read
		assert(!isPlaying()); // no need to update the 'aud' data
...
void Y8950TimAdpcm::writeMemory(unsigned memPtr, byte value)
{
...
byte Y8950TimAdpcm::readMemory(unsigned memPtr) const
{
	unsigned addr = (memPtr / 2) & addrMask;
...
int Y8950TimAdpcm::calcSample(bool doEmu)
{
...
void Y8950TimAdpcm::serialize(Archive& ar, unsigned version)
{
...
INSTANTIATE_SERIALIZE_METHODS(Y8950TimAdpcm);
